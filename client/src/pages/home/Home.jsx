...
const Home = () => {
    ...
    (async () => {
      try {
        const res = await fetch(
          `https://uniconn-chat-app-repo.onrender.com/api/v1/user?page=${mentorpage}&limit=${mentorlimit}`
        );
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(
          `https://uniconn-chat-app-repo.onrender.com/api/v1/colleges?page=${collegepage}&limit=${collegelimit}`
        );
        const data = await res.json();
